...
class SyncQueue {
    ...
    initializeQueue() {
        try {
            // Use Redis connection from our RedisService
            const redisConfig = {
                redis: {
        ...
        try {
            ...
            if (pattern) {
                ...
                if (matchingKeys.length > 0) {
                    await redisService.getClient().del(...matchingKeys);
    ...
    async logFailedJob(job) {
        ...
        await redisService.getClient().lpush('failed-sync-jobs', JSON.stringify(failedJob));
