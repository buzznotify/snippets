...
class DataSyncService {
    ...
    async getUserSyncStats() {
        try {
            const userCount = await cacheService.redis.getClient().keys('user:*').then(keys => keys.length);
            const emailCount = await cacheService.redis.getClient().keys('user:email:*').then(keys => keys.length);
            const sessionCount = await cacheService.redis.getClient().keys('session:*').then(keys => keys.length);
    ...
    async getSnippetSyncStats() {
        try {
            const snippetCount = await cacheService.redis.getClient().keys('snippet:*').then(keys => keys.length);
            const userSnippetsCount = await cacheService.redis.getClient().keys('user_snippets:*').then(keys => keys.length);
    ...
    async verifyDataConsistency() {
        try {
            ...
            const cachedUsers = await cacheService.redis.getClient().keys('user:*').then(keys => keys.length);
            const dbUsers = await cacheService.redis.getClient().keys('user:email:*').then(keys => keys.length);
            results.users = { cached: cachedUsers, db: dbUsers, consistent: cachedUsers > 0 && dbUsers > 0 };

            // Check snippet consistency
            const cachedSnippets = await cacheService.redis.getClient().keys('snippet:*').then(keys => keys.length);
            const dbSnippets = await cacheService.redis.getClient().keys('user_snippets:*').then(keys => keys.length);
